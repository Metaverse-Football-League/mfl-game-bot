...
def update(team, goals, victory):
    ...
    for line in lines:
        ...
        if name == team:
            ...
            newline = name+","+d_match+","+d_goals+","+d_vict+","+at_match+","+at_goals+","+at_vict+","
            replace = line.replace(line, newline)
            line = replace
            find = 1
        l_lines.append(line)
    if found == 0:
        addline = team+","+str(1)+","+str(goals)+","+str(victory)+","+str(1)+","+str(goals)+","+str(victory)+","
...
def reset_file(file):
    ...
    for line in lines:
        ...
        if name != "discordId":
            ...
            newline = name+",0,0,0,"+at_match+","+at_goals+","+at_vict+","
            replace = line.replace(line, newline)
            line = replace
        l_lines.append(line)
    file.seek(0)
